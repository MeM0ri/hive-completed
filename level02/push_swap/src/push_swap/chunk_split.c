...
/*   Updated: 2025/02/04 17:53:19 by alfokin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	chunk_split(t_push_swap *data, t_chunk *chunk, t_split_dest *dest)
{
	...
	while (chunk->size--)
	{
		...
		{
			dest->max.size += move_from_to(data, chunk->loc, dest->max.loc);	// UNTESTED!!!
			split_max_reduction(data, &dest->max);								// UNDONE!!!
			if (a_partly_sort(data, 1) && chunk->size)							// UNDONE!!!
				easy_sort(data, chunk);											// UNDONE!!!
		}
		else if (next_value > max_value - first_pivot)
			dest->mid.size += move_from_to(data, chunk->loc, dest->mid.loc);	// UNTESTED!!!
		else
			dest->min.size += move_from_to(data, chunk->loc, dest->min.loc);	// UNTESTED!!!
	}
}

void	set_pivots(t_push_swap *data, int *first_pivot, int *second_pivot)
{
	if (&data->stack_a.stack && current_stack_size(&data->stack_a.stack) > 1)
	{
		...
		*second_pivot = data->stack_a.stack[data->stack_a.bottom];
		...
		&& current_stack_size(&data->stack_a.stack) == 1)
	{
		*first_pivot = data->stack_a.stack[data->stack_a.top];
